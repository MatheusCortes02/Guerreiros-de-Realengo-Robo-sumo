...
A única função dele é lutar: **liga a chave, espera 5 s, procura o adversário e ataca**. Não tem controle remoto nem recebe comando de ninguém. Chassi impresso em 3D, tração traseira com dois micromotores N20, **ESP32** como cérebro, ponte H **TB6612FNG**, sonar **HC-SR04** para achar o oponente e dois sensores **TCRT5000** para não sair da arena. Tudo fica sob um capô fechado.
...
| Item | Valor |
|---|---|
| Comportamento | autônomo: espera 5 s, procura girando, ataca em linha reta, recua na borda branca |
...
| Sensores | 1 × HC-SR04 (adversário) · 2 × TCRT5000 (borda) |
...
| Fiação | 31 fios · 2 emendas · nenhuma barra de pinos |
...
- lista de corte dos 31 fios, com cor, bitola, comprimento e por onde cada um passa;
- ordem de solda e checagem com multímetro antes de ligar a bateria;
- o firmware com botão de copiar.

O vídeo mostra a versão com módulo TCRT e ainda tem o receptor infravermelho, que saiu do projeto. Os pontos exatos de solda estão nos desenhos do manual e deste README.
...
Depois de imprimir o chassi, teste um transdutor do HC-SR04 nos dois furos centrais da frente: tem que entrar com a mão. Os dois furos externos Ø15 da frente ficam vazios.
...
| Chave gangorra 10 A | 1 | furo 13 × 9 mm · é ela que dá a largada |
...
| Fio flexível 26 AWG | ≈ 2,5 m | sinais e 3,3 V |
...
- **Um pino de 3V3 alimenta três componentes** porque o fio se divide numa emenda. O limite é a corrente: tudo no 3,3 V soma cerca de 125 mA.
- **GND é um ponto só.** Os furos GND de cada placa já são ligados por dentro; os fios unem as placas. Qualquer furo GND serve.
- **Nunca 5 V num pino do ESP32.** O ECHO do HC-SR04 passa por um divisor e os TCRT ficam no 3,3 V.
...
| GPIO 2 | LED azul da placa | pisca nos 5 s de espera |
| VIN | LM2596 OUT+ | 5 V |
...
A chave é o único "comando" do robô: ligou, começa a contagem de 5 s.
...
|---|---|---|
| R1 · grande da frente | 3V3 e GND das emendas, TRIG, ECHO e os sinais dos dois TCRT | fileira da frente do ESP32; D34 e D35 correm por baixo dele até a fileira de trás |
...
A lista de corte completa dos 31 fios está no manual.

## Firmware

Código em [`firmware/guerreiros_de_realengo/guerreiros_de_realengo.ino`](firmware/guerreiros_de_realengo/guerreiros_de_realengo.ino). Não usa nenhuma biblioteca extra.

### Preparar a Arduino IDE

1. Gerenciador de Placas: instale **esp32** da Espressif (versão 3.3.11).
2. Selecione a placa **DOIT ESP32 DEVKIT V1** e a porta USB.
...
3. Robô no preto: a leitura da linha fica alta (ex.: 3000). Na borda branca, baixa (ex.: 300).
4. Passe só o sensor do lado esquerdo sobre o branco: tem que mudar `esq`. Se mudar `dir`, os fios de D34 e D35 estão trocados.
5. Aproxime a mão da frente do robô: a distância tem que cair.
6. Com o **robô no ar**, digite no Monitor Serial:
...
7. Coloque `MODO_TESTE = false` e regrave.

### Na luta

1. Robô no centro do dohyô, sobre o preto.
2. Ligue a chave e tire a mão. O LED azul pisca rápido por 5 s.
3. O robô gira procurando o adversário e ataca quando o sonar acha.
4. **Para parar, desligue a chave.**

### Como o robô decide

| Estado | O que faz | Sai quando |
|---|---|---|
| `PREPARANDO` | espera os 5 s depois de ligar e mede o preto dos sensores de linha | passam 5 s |
...
- **Calibração automática:** durante os 5 s de espera o robô mede o preto e usa 55% dessa leitura como limiar de cada sensor. Se a leitura sair baixa demais, usa `LIMIAR_PADRAO`.
...
| `T_PREPARO` | 5000 ms | espera depois de ligar a chave |
...
3. **HC-SR04:** fios, divisor do ECHO e cola quente.
...
| O robô não se mexe depois dos 5 s | `MODO_TESTE` ainda `true` ou ponte H sem 3,3 V no VCC | gravar com `MODO_TESTE = false`; conferir o fio da emenda 3V3 até o VCC |
...
- até **500 g**;
- robô parado por **5 s** depois de acionado.

Confira o regulamento oficial da competição da InovaWeek antes do evento.
